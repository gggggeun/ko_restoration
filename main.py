...
def set_env():
    complex_verb_set = util.create_set(f"{os.path.abspath(__file__).split('main.py')[0]}ko_restoration/complex_verb.txt")
    mecab = Mecab()
    return mecab, complex_verb_set

def start_restoration(tok, complex_verb_set, lines):
    results = ''
    for row in lines:
        body = row
